...
import { resolveBuildPillPreviewLabel, useBuildPillPreview } from "../../buildPillPreview";
        ...
        {pillLabel ? (
          <div className="ml-1 hidden h-7 shrink-0 items-center @[15rem]/sidebar-header:flex md:flex">
            <Badge
              ...
              variant="secondary"
            >
              {import.meta.env.DEV ? <DevBuildPillLabel label={pillLabel} /> : pillLabel}
            </Badge>
          </div>
        ) : null}
      </div>
...
function DevBuildPillLabel({ label }: { label: EnvironmentIdentificationPillLabel }) {
  const [pillPreview] = useBuildPillPreview();
  return resolveBuildPillPreviewLabel(label, pillPreview);
}
